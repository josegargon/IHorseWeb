...
/**
 * Theme Settings documentation page functions.
 *
  ...
  function ot_type_creating_options() {
    ...
    echo '<div class="format-setting type-textblock wide-desc">';
      ...
      echo '<div class="description">';
        ...
        echo '<p>' . __( 'Enter a detailed description for the users to read on the Theme Settings page, HTML is allowed. This is also where you enter content for both the Textblock & Textblock Titled option types.', 'option-tree' ) . '</p>';
        ...
        echo '<p>' . __( 'Add a comma separated list of post type like <code>post,page</code>. This will only affect the following option types: Custom Post Type Checkbox, & Custom Post Type Select. Below are the default post types available with WordPress and that are also compatible with Theme Settings. You can also add your own custom <code>post_type</code>. At this time <code>any</code> does not seem to return results properly and is something I plan on looking into.', 'option-tree' ) . '</p>';
  ...
  function ot_type_option_types() {
    ...
    echo '<div class="format-setting type-textblock wide-desc">';
      ...
      echo '<div class="description">';
        ...
        echo '<p>' . __( 'The CSS option type is a textarea that when used properly can add dynamic CSS to your theme from within Theme Settings. Unfortunately, due server limitations you will need to create a file named <code>dynamic.css</code> at the root level of your theme and change permissions using chmod so the server can write to the file. I have had the most success setting this single file to 0777 but feel free to play around with permissions until everything is working. A good starting point is 0666. When the server can save to the file CSS will automatically be updated each time you save your Theme Settings.', 'option-tree' ) . '</p>';
...
add_filter( \'ot_recognized_font_families\', \'filter_ot_recognized_font_families\', 10, 2 );</code></pre>';
        ...
        echo '<p>' . __( 'The Upload option type is used to upload any WordPress supported media. After uploading, users are required to press the "Send to Theme Settings" button in order to populate the input with the URI of that media. There is one caveat of this feature. If you import the Theme Settings and have uploaded media on one site the old URI will not reflect the URI of your new site. You\'ll have to re-upload or FTP any media to your new server and change the URIs if necessary.', 'option-tree' ) . '</p>';
  ...
  function ot_type_examples() {
    ...
    echo '<div class="format-setting type-textblock wide-desc">';
      ...
      echo '<div class="description">';
        ...
        echo '<p class="aside">' . __( 'If you\'re using the plugin version of Theme Settings it is highly recommended to include a <code>function_exists</code> check in your code, as described in the examples below. If you\'ve integrated Theme Settings directly into your themes root directory, you will <strong>not</strong> need to wrap your code with <code>function_exists</code>, as you\'re guaranteed to have the <code>ot_get_option()</code> function available.', 'option-tree' ) . '</p>';
  ...
  function ot_type_layouts_overview() {
    ...
    echo '<div class="format-setting type-textblock wide-desc">';
      ...
      echo '<div class="description">';
        ...
        echo '<p>' . __( 'Layouts make your theme awesome! With Theme Settings data that you can save/import/export you can package themes with different color variations, or make it easy to do A/B testing on text and so much more. Basically, you save a snapshot of your data as a layout.', 'option-tree' ) . '</p>';
        ...
        echo '<ul class="docs-ul">';
          echo '<li>'. __( 'Go to the <code>Appearance->Theme Settings</code> page.', 'option-tree' ) . '</li>';
          echo '<li>' . __( 'Modify and save your Theme Settings and the layout will be updated automatically.', 'option-tree' ) . '</li>';
          echo '<li>' . __( 'Saving Theme Settings data will update the currently active layout, so before you start saving make sure you want to modify the current layout.', 'option-tree' ) . '</li>';
          echo '<li>' . __( 'If you want to edit a new layout, first create it then save your Theme Settings.', 'option-tree' ) . '</li>';
        echo '</ul>';
        ...
        echo '<h5>' . __( 'Edit Layout Data', 'option-tree' ) . ':</h5>';
        echo '<ul class="docs-ul">';
          echo '<li>'. __( 'Go to the <code>Appearance->Theme Settings</code> tab.', 'option-tree' ) . '</li>';
          echo '<li>' . __( 'Modify and save your Theme Settings and the layout will be updated automatically.', 'option-tree' ) . '</li>';
          echo '<li>' . __( 'Saving Theme Settings data will update the currently active layout, so before you start saving make sure you want to modify the current layout.', 'option-tree' ) . '</li>';
        echo '</ul>';
...
load_template( trailingslashit( get_template_directory() ) . \'includes/meta-boxes.php\' );
        ...
        echo '<ul class="docs-ul">';
          echo '<li>' . __( 'Add a variation of the following code to your <code>meta-boxes.php</code>. You\'ll obviously need to fill it in with all your custom array values. It\'s important to note here that we use the <code>admin_init</code> filter because if you were to call the <code>ot_register_meta_box</code> function before Theme Settings was loaded the sky would fall on your head.', 'option-tree' ) . '</li>';
        echo '</ul>';
  ...
  function ot_type_theme_mode() {
    ...
    echo '<div class="format-setting type-textblock wide-desc">';
      ...
      echo '<div class="description">';
        ...
        echo '<p>' . __( 'There are a few simple steps you need to take in order to use Theme Settings as a theme included module. In the code below I\'ll show you a basic demo of how to include the entire plugin as a module, which will allow you to have the most up-to-date version of Theme Settings without ever needing to hack the core of the plugin. If you would like to see some demo code, there is a directory named <code>theme-mode</code> inside the <code>assets</code> directory that contains a file named <code>demo-theme-options.php</code> you can reference.', 'option-tree' ) . '</p>';
        
        echo '<h5>' . __( 'Step 1: Include the plugin & turn on theme mode.', 'option-tree' ) . '</h5>';
        echo '<ul class="docs-ul">';
          echo '<li>'. __( 'Download the latest version of <a href="http://wordpress.org/extend/plugins/option-tree/" rel="nofollow" target="_blank">Theme Settings</a>.', 'option-tree' ) . '</li>';
 ...
 * This will hide the "New Layout" section on the Theme Settings page.
 */
...
/**
 * Required: include Theme Settings.
 */
load_template( trailingslashit( get_template_directory() ) . \'option-tree/ot-loader.php\' );
</code></pre>';
        
        echo '<p class="aside">' . __( 'It\'s that simple! You now have Theme Settings built into your theme and anytime there\'s an update to the plugin you just replace the old version and you\'re good to go..', 'option-tree' ) . '</p>';
        
        echo '<h5>' . __( 'Step 2: Create Theme Settings without using the UI Builder.', 'option-tree' ) . '</h5>';
        echo '<ul class="docs-ul">';
...
load_template( trailingslashit( get_template_directory() ) . \'includes/theme-options.php\' );
        ...
        echo '<p>' . __( 'The code below is a boilerplate to get your started. For a full list of the available option types click the "Option Types" tab above. Also a quick note, you don\'t need to put Theme Settings in theme mode to manually create options but you will want to hide the docs and settings as each time you load the admin area the settings be be written over with the code below if they\'ve changed in any way. However, this ensures your settings do not get tampered with by the end-user.', 'option-tree' ) . '</p>';
...
/**
 * Build the custom settings & update Theme Settings.
 */
function custom_theme_options() {
   ...
   * passes to the Theme Settings Settings API Class.
   */
